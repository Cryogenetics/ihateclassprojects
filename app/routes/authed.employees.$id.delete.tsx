...
export const action = async ({params}: ActionFunctionArgs) => {
    ...
    try {
        ...
        if (appointments[0].count > 0) {
            return {
                error: "Cannot delete mechanic with existing appointments. Reassign or cancel appointments first."
            };
        }

        // If no appointments, proceed with deletion
        await makeDBQuery("DELETE FROM mechanic WHERE employee_id = ?", [parseInt(id)]);
        return {success: true};
    } catch (error) {
                    ...
                    <Form method="post" className="space-y-6">
                        ...
                        <div className="bg-white p-4 border border-gray-200 rounded-lg">
                            ...
                            <div className="space-y-2">
                                ...
                                <p><span className="font-medium">Specialty:</span> {mechanic.specialty}</p>
                            </div>
                        </div>
